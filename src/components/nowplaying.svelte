<script>
	...
	async function getNowPlaying() {
		song = await fetch('http://localhost:3000/api/now_playing.json').then((res) => res.json());
	}
	onMount(async () => {
		getNowPlaying();
	});
...
<div class="text-sm w-fit text-gray-500 bg-gray-100 border-2 p-4 pl-5 py-2 rounded-lg">
	{#if !song}
		<svg
			class="inline h-4 w-4 top-0 -translate-y-[1.5px]"
			fill="currentColor"
			viewBox="0 0 32 32"
	...
	<!-- <a
		href="/dashboard"
		sveltekit:prefetch
		class="text-xs text-primary hover:underline flex justify-end text-right"
		><svg
			xmlns="http://www.w3.org/2000/svg"
			class="h-3 w-3 inline-block bottom-0 translate-y-[0.45rem]"
			fill="none"
			viewBox="0 0 24 24"
			stroke="currentColor"
			stroke-width="3"
